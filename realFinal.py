...
while (True):
    ...
    cv2.imshow("handimagehsv", img_hsv)
    #살색 히스토그램에 대한 Range적용
    img_hand = cv2.inRange(img_hsv, lower, upper)
    
    cv2.imshow("handimage", img_hand)
    ...
    #3 - Affine
    frame = cv2.warpPerspective(image_rotated,matrix,(IMAGE_WIDTH,IMAGE_HEIGHT))
    result = cv2.warpPerspective(image_rotated1,matrix,(IMAGE_WIDTH,IMAGE_HEIGHT))
    #4 - Gary
    ...
    if isBgCaptured == 1:  # this part wont run until background captured
        ...
        if length > 1:
            for i in range(length):  # find the biggest contour (ording to area)
                ...
                elif (maxArea > area) & (area > secondMaxArea):
                    secondMaxArea = area
                    cii = i
            ...
            secondRes = contours[cii]
            secondHull = cv2.convexHull(secondRes)
            
            drawing = np.zeros(img.shape, np.uint8)
                
            ret, points = getFingerPosition(res,drawing,debug=False)
            secondRet, secondPoints = getFingerPosition(secondRes,drawing,debug=False)
            
            firstFingerIndex = 0 # finger Number Index
            secondFingerIndex = 5
            ...
            if ret > 0 and len(points) > 0:  
                ...
                for point in points:
                    ...
                    cv2.putText(drawing, str(firstFingerIndex), point, cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,255,255), thickness=2)
                    print(firstFingerIndex + 1, "번째 손가락 : ", point)
                    firstFingerIndex = firstFingerIndex + 1
  
            if secondRet > 0 and len(secondPoints) > 0:  
                secondPoints.sort()
                for secondPoint in secondPoints:
                    cv2.circle(drawing, secondPoint, 20, [ 255, 0, 255], 5)
                    cv2.putText(drawing, str(secondFingerIndex), secondPoint, cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255,255,255), thickness=2)
                    print(secondFingerIndex + 1, "번째 손가락 : ", secondPoint)
                    secondFingerIndex = secondFingerIndex + 1
            ...
            cv2.drawContours(drawing, [secondRes], 0, (0, 255, 0), 2)
            cv2.drawContours(drawing, [secondHull], 0, (0, 0, 255), 3)
